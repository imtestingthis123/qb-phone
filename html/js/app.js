...
QB.Phone.Functions.NewApp = function(appData) {
    const container = document.querySelector('.phone-application-container');
    if (document.querySelector('.' + appData.name + '-app')) {
        document.querySelector('.' + appData.name + '-app').remove();
    }
    let app = document.createElement('div');
    app.classList.add(appData.name + '-app');
    app.style.padding = '0.5vh';
    container.append(app);

    let header = document.createElement('div');
    header.classList.add(appData.name + '-header');
    header.style.marginTop = '5vh';
    header.innerHTML = appData.title;
    app.append(header);

    let headerTop = document.createElement('div');
    headerTop.classList.add(appData.name + '-header-top');
    header.append(headerTop);

    let headerFooter = document.createElement('div');
    headerFooter.classList.add(appData.name + '-header-footer');
    header.append(headerFooter);
    app.append(appData.app);
...
$(document).on('click', '.phone-application', function(e){
    ...
    if (PressedApplication == "notificationhistory") {
        AppObject = QB.Phone.Functions.NewApp({
            name: "notificationhistory",
            title: "Notification History",
            app: PhoneNotificationHistory(NotificationList)
        })
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                else if (PressedApplication == "notificationhistory") {
